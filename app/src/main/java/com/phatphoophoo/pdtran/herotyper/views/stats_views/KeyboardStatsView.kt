...
class KeyboardStatsView:
    ...
    //TODO move logic to presenter with key status
    //Place key accuracy
    fun initKeyColors(keyStats: Map<String, Pair<Int, Int>>) {
        for(keyButtonId in keyButtonIds) {
            ...
            if(total < ANALYSIS_THRESHOLD_COUNT) {
                ...
                button.background.colorFilter = LightingColorFilter(KeyColor.softGray.value, 0)
            } else if(ratio > 2) {
                button.background.colorFilter = LightingColorFilter(KeyColor.softGreen.value, 0)
            } else if(ratio > 1) {
                button.background.colorFilter = LightingColorFilter(KeyColor.softYellow.value, 0)
            } else {
                button.background.colorFilter = LightingColorFilter(KeyColor.softRed.value, 0)
            }
